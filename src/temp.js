...
export default class PlayingScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.input.on("pointerdown", (pointer) => {
            ...
            this.socket.emit("shootBeam", {
                ...
                playerId: this.m_player.id, //playerID
            });
        });
    ...
    handlePlayerCollision(player1, player2) {
        console.log(
            `Collision detected between Player ${player1.id} and Player ${player2.id}`
        );
    ...
    syncTimeWithServer() {
        ...
        this.socket.emit("requestServerTime", clientTime, (serverTime) => {
            ...
            console.log(
                "Server time synchronized. Offset:",
    ...
    update() {
        if (this.m_player) {
            ...
            this.m_background.tilePositionX =
                this.m_player.x - Config.width / 2;
            this.m_background.tilePositionY =
                this.m_player.y - Config.height / 2;

            //+hp+ HP텍스트 위치 및 내용 업댓
            if (this.m_player.hpText) {
                this.m_player.hpText.setPosition(
                    this.m_player.x,
                    this.m_player.y - 20
                );
    ...
    setupSocketListeners() {
        this.socket.on("state", (state) => {
            Object.keys(state).forEach((playerId) => {
                ...
                if (!this.otherPlayers[playerId]) {
                    const newPlayer = this.physics.add.image(
                        x,
                        y,
                        "playerTexture"
                    );
        ...
        this.socket.on("shootBeam", (data) => {
            if (data.playerId !== this.m_player.id) {
                //수신된 발사체 정보가 현재 플레이어가 아닐 때만 화면에 표시(이중 출력 방지)
                this.createBeam(data.x, data.y, {
                    x: data.targetX,
                    y: data.targetY,
                });
    ...
    updateOtherPlayers() {
        Object.values(this.otherPlayers).forEach((player) => {
            ...
            //hp없을 시 생성
            if (!player.hpText) {
                player.hpText = this.add
                    .text(player.x, player.y - 20, `HP: ${player.hp || 100}`, {
                        font: "16px Arial",
                        fill: "#ff0000",
                    })
                    .setOrigin(0.5);
